/*
 * main.cpp - CCS/MSP430G2553 bare-metal test runner for GRU HAR.
 *
 * Mirrors msp/ccs_fastgrnn_har/main.cpp (16 MHz DCO, USCI_A0 9600 baud UART,
 * Timer_A 1 ms tick) so the GRU vs FastGRNN vs LSTM latency/energy comparison
 * is apples-to-apples. Only latency + energy modes are kept (no MPU6050/live).
 *
 * TEST_MODE:
...
//   TEST_MODE == 4  LIVE-LATENCY: end-to-end per-sample latency (I2C read + gru_step),
...
static void run_live_latency(void) {
    sprint("Mode: LIVE end-to-end latency (MPU6050 + gru_step, 50Hz)\n");
    sprint("H="); sprint_u(HIDDEN_SIZE); sprint(" window="); sprint_u(WINDOW_T);
    sprint(" (LUT = gru.cpp ayari; ELLE not al)\n");
...
static void run_live_energy(void) {
    ...
    while (1) {
        ...
        if (mpu6050_read_accel(&raw[0], &raw[1], &raw[2]) == 0) {
            float x[3] = { raw[0] * ACCEL_SCALE, raw[1] * ACCEL_SCALE, raw[2] * ACCEL_SCALE };
            gru_step(x);
            if (++sc >= WINDOW_T) { sc = 0; (void)gru_predict(); gru_reset(); }
        }
        while ((millis_ccs() - t0) < 20) { /* pace 50 Hz */ }
    }
}
...
int main(void) {
    ...
    sprint("=================================\n");
    sprint(" GRU HAR - MSP430G2553 CCS 16 MHz\n");
    sprint("=================================\n");

#if TEST_MODE == 3
    sprint("Mode: ENERGY BENCHMARK\n");
    UCA0CTL1 |= UCSWRST;            // silence UART for the ammeter
    P1OUT &= ~BIT0;
    gru_reset();
    while (1) {
  #if BENCH_MODE == 0
