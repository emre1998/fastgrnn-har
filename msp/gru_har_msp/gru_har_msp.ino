/*
 * gru_har.ino - GRU HAR latency + energy measurement harness (Arduino Uno)
 *
 * Mirrors fastgrnn_har.ino's benchmark modes so the GRU vs FastGRNN vs LSTM
 * latency/energy comparison is apples-to-apples. Input values do not affect
 * compute cost, so the benchmarks feed a zero sample.
 *
 * Target: Arduino Uno R3 (ATmega328P, 16 MHz, 32 KB Flash, 2 KB SRAM), 115200 baud.
 *
 * TEST_MODE:
 *   0 = LATENCY  (default): time one gru_step and a full WINDOW_T-step window,
 *                 report microseconds. This backs the "real-time" claim.
 *   3 = ENERGY   benchmark for an external INA226/ammeter (see BENCH_MODE).
 *
 * BENCH_MODE (TEST_MODE==3):
 *   0 = IDLE       (delay(20), baseline)
 *   1 = STREAM50HZ (gru_step every 20 ms, idle rest - realistic HAR)
 *   2 = CONTINUOUS (tight gru_step loop - worst case)
 */
#include "gru.h"
...
void setup() {
    ...
    // Warm-up (fill caches / branch predictors; also matches deployment start).
    gru_reset();
    for (uint16_t t = 0; t < WINDOW_T; t++) gru_step(ZERO);

    // --- Per-step latency: average over one full window ---
    gru_reset();
    ...
    volatile uint8_t cls = gru_predict();
