...
\fastgrnn{} for HAR on bare-metal MCUs and reports three
...
reference across 3{,}399 test windows. This level of
hardware-independent reproducibility is itself an artifact worth
preserving for safety-relevant deployments.
...
target. The recipe is independent of \fastgrnn{} and applies to any
recurrent cell that relies on $\sigma$ or $\tanh$ activations.

\item \textbf{Characterization of recurrent warm-up latency.} We
observe that prediction stability requires approximately 2~seconds
($\sim$100 samples at 50~Hz) of recurrent hidden-state evolution
...
and real-time streaming performance.
