...
requires roughly two seconds ($\sim$100 samples at 50~Hz) of
hidden-state evolution before the emitted class label settles,
...
class around $t{=}100$. This is a property of the recurrent
...
are confirmed only $\sim$2~s after they occur. This is a
non-trivial latency budget and should be reported alongside
per-sample throughput when characterizing on-device RNNs. We
hypothesize the same effect occurs in other gated recurrent cells
of comparable size; verifying this on LSTM/GRU baselines at
matched parameter counts is an obvious follow-up.
...
label is unstable until $\sim$2~s of history accumulates.}
\label{fig:warmup}
\end{figure}
...
\item \textbf{Energy per inference.} We report time-per-inference
but not joules-per-inference. Direct on-board current measurement
would complete the deployment story and tie back to the
motivation of Section~\ref{sec:intro}: silicon-supply and energy
constraints are coupled, and a 30$\times$ wall-clock speedup is
only useful at the application level if it also reduces battery
draw proportionally.
\end{itemize}
