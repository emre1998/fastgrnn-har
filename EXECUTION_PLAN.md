...
- [x] **B2 (K1)** TAMAM (8-9 Tem 2026, B2_RESULTS.md). CCS (Energia değil) kullanıldı — msp/ccs_gru_har + msp/ccs_lstm_har + msp/ccs_uart_test yazıldı. Debug/Flash staleness + proje karışıklığı debug edildi (banner-metni "fingerprint" yöntemiyle çözüldü — TEST_MODE 1'in kendi UART çıktısı "GRU HAR"/"LSTM HAR" + H değeri kesin kanıt verdi). Enerji tamamen baştan ölçüldü (temiz prosedür: Save→Clean→Build→Debug, hep aynı yöntem).
  ...
  DERLEYİCİ -O TAM SWEEP (ÖLÇÜLEN, 9 Tem akşam, 36 ölçüm = 3 hücre × LUT{0,1} × -O{off,0,1,2,3,4}): no-LUT -O'ya neredeyse ETKİSİZ (GRU %4.6, LSTM ~%0, FastGRNN ~%4); LUT'ta -O ılımlı kazanç (%5.5-8.9), HER hücrede -O2'de doyuyor. per-step no-LUT plato: GRU 19.273 / LSTM 22.800 / FastGRNN ~26.1ms; LUT plato: GRU 12.116 / LSTM 12.371 / FastGRNN 13.9ms. KÖK SEBEP: MSP430 FPU YOK → tüm float aritmetiği (MAC+expf/tanhf) önceden-derlenmiş RTS soft-float, -O dokunamaz — "MCU neden yavaş" ile "-O neden işe yaramaz" aynı kök (tezle örtüşür). Deploy -O3=plato, -O2 zaten yeter. use_hw_mpy=none.
  ⚠️ 54s KESİN GÖMÜLDÜ (2 kanıt): (a) FastGRNN'in KENDİ kodunda -O off=~27ms/step ölçüldü, 54s değil (–O tavanı %9); (b) kullanıcı doğruladı: 54s = Week 8'de MPU6050 SENSÖR döngüdeyken live-mode deneyi (I2C+eski kod), saf-hesaplama latency'si DEĞİL. 54s SÜTUN YAPILMAZ → "sensörlü live-mode, non-comparable" dipnotu. 30.5×/96.7% ana tablodan çıkar. −%46 (FastGRNN ~26ms, -O-bağımsız) sağlam.
  SENSÖRLÜ GERÇEK-DÜNYA (TAMAM, 10 Tem 2026, 24 ölçüm = 3 hücre × LUT{0,1} × I2C{10k,100k} × {lat,enerji}; B2_RESULTS.md). Live harness üçüne de eklendi (TEST_MODE 4=lat/5=enerji, I2C_FAST makrosu, MPU6050 USCI_B0 sürücüsü). BULGU: real-time SADECE LUT+100kHz'de (GRU 12.01<LSTM 12.98<FastGRNN 13.98ms OK); başka HİÇBİR kombinasyon değil. İKİ BAĞIMSIZ ön-koşul: LUT VE hızlı sensör I2C. 10kHz sensör okuma 8.4ms → LUT'lu en hızlı bile FAIL; 100kHz=0.8ms. no-LUT her I2C'de FAIL (inference≥19ms). Sistem gücü hücre/LUT-bağımsız ~32mW(100k)/~34mW(10k) = bench MCU-only'nin ~2×'i (MPU6050 +14mW). Cross-val: her live inference bench zero-input ile ±%5. Flaky-kontak yakalandı (bir GRU-100k enerji 40mW salınım=I2C timeout spin, reseat→32mW). Commit'ler: 8592fd→(sensör matris).
